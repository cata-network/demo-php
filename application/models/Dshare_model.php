...
//        $ssh_pem = file_get_contents($file_path);
    ...
    {
        $sql = "select  `id` ,`email`,`title`,`brief` ,`price` ,`img_url`,`fetch_time` 
                from market where status = 1 limit $start,$limit";
//        echo $sql;
        $result = $this->db->query($sql)->result_array();
        $sql_num ="select  count(0) as num from market where status = 1";
        $num = $this->db->query($sql_num)->result_array();
        return array("status" => 0, "msg" => "success", "num" => $num[0]['num'], "results" =>$result);
    }
    ...
    public function get_mynote($email, $start, $limit)
    {
        ...
        $result = $this->db->query($sql)->result_array();
        return array("status" => 0, "msg" => "success", "count"=>$num[0]['num'], "results" =>$result);
    }
    ...
    //获取余额(暂未用)
     ...
     public function get_balance($email)
     {
        ...
        $sql = "select  sum(balance) as balance  from
               (select *, null as effect_time  from `user_balance`  where `user_email` ='$email'
                union
                select * from `user_balance_delay` where `user_email` ='$email' and effect_time<='$fetch_time')  as b ";
        $result = $this->db->query($sql)->result_array();
        return array("status" => 0, "msg" => "success", "results" =>$result);
     }
    /****************交易*******************/
    ...
    //获取用户交易记录（交易包括买卖）
    public function get_user_orders($email, $start, $limit)
    {
        $sql_num="select count(*) as num from item_transaction
                LEFT JOIN market ON
                `item_transaction` .note_id= `market` .id
